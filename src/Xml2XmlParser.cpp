...
namespace {
void mlStartElement(void* userData, const xmlChar* localname,
                    const xmlChar* prefix, const xmlChar* nsUri,
                    int numNamespaces, const xmlChar** namespaces, int numAttrs,
                    int numDefaulted, const xmlChar** attrs)
{
  ...
  sd->psm->beginElement(reinterpret_cast<const char*>(localname),
                        reinterpret_cast<const char*>(prefix),
                        reinterpret_cast<const char*>(nsUri), xmlAttrs);
...
namespace {
void mlEndElement(void* userData, const xmlChar* localname,
                  const xmlChar* prefix, const xmlChar* nsUri)
{
  SessionData* sd = reinterpret_cast<SessionData*>(userData);
  ...
  sd->psm->endElement(reinterpret_cast<const char*>(localname),
                      reinterpret_cast<const char*>(prefix),
                      reinterpret_cast<const char*>(nsUri), characters);
}
} // namespace
...
XmlParser::XmlParser(ParserStateMachine* psm)
    : psm_(psm),
      sessionData_(psm),
      ctx_(xmlCreatePushParserCtxt(&mySAXHandler, &sessionData_, nullptr, 0,
                                   nullptr)),
      lastError_(0)
{
}

XmlParser::~XmlParser() { xmlFreeParserCtxt(ctx_); }
